 ...
 */
  ...
  /**
   * Calculate total current (in Ampere) that is drained from this
   * EnergySource. This function is called by EnergySource::UpdateEnergySource
   * to calculate how much energy is drained from this EnergySource since
   * last update.
   *
   * The total current is calculated by summing up the current of all
   * DeviceEnergyModels installed on the node and subtracting the current
   * generated by all EnergyHarvesters installed on the node.
   *
   * Positive current means energy is drained from the EnergySource.
   * Negative current means energy is fed to the EnergySource.
   *
   * \return total current (in Ampere) drained from (or fed to) this EnergySource.
   */
double
