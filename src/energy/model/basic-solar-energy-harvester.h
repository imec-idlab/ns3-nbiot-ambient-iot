...
#ifndef BASIC_SOLAR_ENERGY_HARVESTER
#define BASIC_SOLAR_ENERGY_HARVESTER
...
/**
 * \ingroup energy
 * BasicSolarEnergyHarvester increases remaining energy stored in an associated
 * Energy Source. The BasicSolarEnergyHarvester implements a simple model in which
 * the amount of power provided by the simulated solar panels harvester varies over time according
 * the time of the day.
 *
 ...
 *
 */
class BasicSolarEnergyHarvester: public EnergyHarvester
{
public:
  static TypeId GetTypeId (void);

  BasicSolarEnergyHarvester ();

  /**
   * \param updateInterval Energy harvesting update interval.
   *
   * BasicSolarEnergyHarvester constructor function that sets the interval
   * between each update of the value of the power harvested by this
   * energy harvester.
   */
  BasicSolarEnergyHarvester (Time updateInterval);

  virtual ~BasicSolarEnergyHarvester ();
...
private:
  ...
  /**
   * \returns m_harvestedPower The power currently provided by the Basic Solar Energy Harvester.
...
#endif /* defined(BASIC_SOLAR_ENERGY_HARVESTER) */
